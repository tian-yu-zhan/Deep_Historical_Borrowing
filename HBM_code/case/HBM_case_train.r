...
train.rate.hist.vec = c(0.8, 0.65) ## historical studies training response rate of two endpoints
n.endpoint = length(train.rate.hist.vec) ## number of endpoints
n.cluster= 4 ## number of cores for parallel computing
set.seed(12) ## set random seed for generating training data
...
n.train = 16000 ## training data size
...
## vector of historical data from literature
fix.resp.hist.vec = c(200, 249, 52, 160, 202, 44)

## matrix of historical data, row as endpoints, column as historical studies
fix.resp.hist.mat = matrix(fix.resp.hist.vec, nrow = n.endpoint, ncol = n.hist,
                           byrow = TRUE)
...
print(mean(fix.resp.hist.mat[1,]/n.vec.hist-0.8))
print(mean(fix.resp.hist.mat[2,]/n.vec.hist-0.65))

## write historical data to file
write.csv(fix.resp.hist.mat, file = paste0(sub_folder_name, "/HBM_hist_data_case.csv"),
          row.names = FALSE)
...
for (ind.train in 1:n.train){
  ...
  train.rate.new.vec.temp = c(runif(1, 0.65, 0.95), runif(1, 0.5, 0.8))
  ...
  if (ind.train<=(1*n.train/4)){
    ...
    train.rate.delta.vec.temp = c(0, runif(1, -0.1, 0.1))
  } else if (ind.train<=(3*n.train/4)){
    train.rate.delta.vec.temp = c(runif(1, -0.1, 0.1), 0)
  } else {
    train.rate.delta.vec.temp = c(runif(1, -0.1, 0.1), runif(1, -0.1, 0.1))
  }
...
label.first.train = foreach(ind.train=1:n.train) %dopar% { 
  ...
  ## number of chains * number of iterations per chain, i.e., 3*10,000.
...
write.csv(data.train.pred.com, file = paste0(sub_folder_name, "/HBM_train_label_case.csv"))
...
for (ind.second.train in 1:n.second.train){
  ...
  if (ind.second.train<=(n.second.train/3)){
    ...
    rate.second.delta.temp = c(0, runif(1, -0.1, 0.1))
  } else {
    rate.second.delta.temp = c(runif(1, -0.1, 0.1), 0)
  }
  
  ## write vector to training data matrix
  rate.second.pbo.temp = c(runif(1, 0.65, 0.95), runif(1, 0.5, 0.8))
...
DNN_scale_parameters = list("col_means_first_mean" = col_means_train_mean, 
                       ...
                       "time_1_diff" = time.1.diff,
                       "time_2_diff" = time.2.diff
)
