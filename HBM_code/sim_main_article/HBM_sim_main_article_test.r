...
fix.resp.hist.mat = 
  read.csv(paste0(sub_folder_name, "/HBM_hist_data_", cor.parameter, ".csv"),
           sep=",")
...
n.output.table = 11 # number of validation scenarios
n.test.itt = 10^1 # number of simulation iterations
n.test.mean.itt = 10^1
...
for (ind.output.table in c(1:11)){
  
  set.seed(2)
  print(ind.output.table)
  
  ## first three have different pbo rates under global null
  if (ind.output.table==1){test.prop.pbo.vec = c(0.3, 0.2); test.prop.delta.vec = c(0, 0)}
  if (ind.output.table==2){test.prop.pbo.vec = c(0.4, 0.3); test.prop.delta.vec = c(0, 0)}
  if (ind.output.table==3){test.prop.pbo.vec = c(0.5, 0.4); test.prop.delta.vec = c(0, 0)}
  
  ## single null
  if (ind.output.table==4){test.prop.pbo.vec = c(0.4, 0.3); test.prop.delta.vec = c(0.1, 0)}
  if (ind.output.table==5){test.prop.pbo.vec = c(0.4, 0.3); test.prop.delta.vec = c(0, 0.1)}
  
  ## alternatives
  if (ind.output.table==6){test.prop.pbo.vec = c(0.3, 0.2); test.prop.delta.vec = c(0.1, 0.1)}
  if (ind.output.table==7){test.prop.pbo.vec = c(0.3, 0.2); test.prop.delta.vec = c(0.12, 0.12)}
  
  if (ind.output.table==8){test.prop.pbo.vec = c(0.4, 0.3); test.prop.delta.vec = c(0.1, 0.1)}
  if (ind.output.table==9){test.prop.pbo.vec = c(0.4, 0.3); test.prop.delta.vec = c(0.12, 0.12)}
  
  if (ind.output.table==10){test.prop.pbo.vec = c(0.5, 0.4); test.prop.delta.vec = c(0.1, 0.1)}
  if (ind.output.table==11){test.prop.pbo.vec = c(0.5, 0.4); test.prop.delta.vec = c(0.12, 0.12)}
  ...
  ## MAP
  if (cor.parameter==0){
    ## for 0.5, 0.4 and correlation parameter of 0
    MAP.norobust.cutoff = 0.9977 
    MAP.robust.1.cutoff = 0.9927 # 50% non-informative prior
    MAP.robust.2.cutoff = 0.9867 # 80% non-informative prior
  } else if (cor.parameter==0.5){
    ## for 0.5, 0.4 and correlation parameter of 0.5
    MAP.norobust.cutoff = 0.997
    MAP.robust.1.cutoff = 0.992 # 50% non-informative prior
    MAP.robust.2.cutoff = 0.986 # 80% non-informative prior
  }
  ...
  for (endpoint.ind in 1:n.endpoint){
    ...
    MAP.rate.norobust.vec = sapply(1:n.test.mean.itt, function(test.itt){
    ...
    MAP.rate.robust.1.vec = sapply(1:n.test.mean.itt, function(test.itt){
    ...
    MAP.rate.robust.2.vec = sapply(1:n.test.mean.itt, function(test.itt){
...
write.csv(output.table, paste0(sub_folder_name, "/HBM_results_", cor.name, ".csv"))
